...
public class Log {
	...
	public Log(boolean useCurrent) {
		...
		File logFile = new File(System.getProperty("user.home") + "/Binocular-server/logs/" + this.today + ".log");
		File tempFile = new File(System.getProperty("user.home") + "/Binocular-server/logs/.logtmp");
		int i = 0;
		while (logFile.exists()) {
			
			i++;
			logFile = new File(System.getProperty("user.home") + "/Binocular-server/logs/" + this.today + "(" + i + ")" + ".log");
		}
		if (useCurrent) {
			if (i > 0) {
				i--;
				if (i == 0) {
					logFile = new File(System.getProperty("user.home") + "/Binocular-server/logs/" + this.today + ".log");
				} else {
					logFile = new File(System.getProperty("user.home") + "/Binocular-server/logs/" + this.today + "(" + i + ")" + ".log");
				}
			}
			...
			try {
				File logDir = new File(System.getProperty("user.home") + "/Binocular-server/logs/");
				logDir.mkdirs();
